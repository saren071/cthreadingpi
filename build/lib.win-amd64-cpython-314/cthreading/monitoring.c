...
#ifdef _WIN32
    /* Windows: TryEnterCriticalSection returns non-zero (TRUE) on success */
    failed = (ct_mutex_trylock(&self->lock) == 0);
#else
    /* POSIX: pthread_mutex_trylock returns 0 on success, error code (EBUSY) on failure */
    failed = (ct_mutex_trylock(&self->lock) != 0);
#endif
